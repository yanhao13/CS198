...
const mongoose = require('mongoose')/* YOUR CODE HERE */
...
async function connect() {
  await mongoose.connect("mongodb+srv://yanhao:Bai13821820539@cluster0.2zkz0st.mongodb.net/") // QUESTION 2. Paste your connection string here, but replace <password> with your password.
...
app.get("/cards", async (req, res) => {
    ...
    const foundCards = await Flashcard.find()/* YOUR CODE HERE */
    res.send(foundCards)
})
...
app.get("/card/:id", async (req, res) => {
    let id = req.params.id
    const foundCard = await Flashcard.findById(id)/* YOUR CODE HERE */
    res.send(foundCard)
})
...
app.get("/delete/:id", async (req, res) => {
    let id = req.params.id
    const foundCard = await Flashcard.findByIdAndDelete(id)/* YOUR CODE HERE */
    res.send(foundCard)
})
